...
int DES::bin_to_dec(string bin){	
	int dec=0,incr=0,sum=0;
	for(int i=(bin.length()-1);i>=0;i--){
		dec=bin[i]-'0';
		sum=sum+(int)(dec*(pow(2.0,incr)));
		incr++;
	}
	return sum;	
}

string DES::dec_to_bin(int dec){
	int i=dec;
	string str="";
	char binary[4];
	while(1){
...
string DES::process_string(string substring,int s[][4][16],int p){
	string row,col;
	for(int i=0;i<6;i++){
...
string DES::fifty_six_bit_key(string key,int pc1[8][7]){
...
string DES::rol_keys(string key,int rotating_schedule[16],int i){	
...
string DES::fourty_eight_bit_key(string C,string D,int pc2[6][8]){		
...
string DES::initial_permuted_msg(string msg,int ip[8][8]){	
...
string DES::expanded_msg(string R,int ep[8][6]){
...
string DES::substitution_function(string xored_message,int sbox[][4][16]){
...
string DES::permuted_message(string thirty_two_bit_msg,int pf[4][8]){
...
string DES::inverse_ip_msg(string reversed_msg,int inv_ip[8][8]){
...
string DES::hex_conversion(string de_msg,string conv_table[23],string hex[23]){
	int len=4;
...
string DES::des_process(string message,string key,bool encrypt){
...
string DES::intToHex(int a)
{
	char cc = (unsigned char)a;
	string ret = "";
	int ost, b,c;
	do
	{
...
string DES::CheckMessage(string message)
{
	if(message.length() == 0) return "fail0";
...
string DES::MessageToHex (string message)
{
	int a = message.length();
	string ret = "";
	for(int i = 0; i < message.length(); i++)
		ret += intToHex((unsigned char)(message[i]));
